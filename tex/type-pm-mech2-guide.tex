...
\date{2026年8月21日版\\
  基準実装：現行\code{type-pm-mech2}実装と\code{README.md}}
...
実装は段階に分けられている．2026年8月21日時点の要点は表\ref{tab:milestones}のとおりである．
...
M3 & data，primitive，signature & 完了 & Bool/List，値を作るconstructor，組込み演算primitive，\code{ifE}，有限signatureと，M2--M3全域の公開静的定理を検証済み．\\
M4 & pattern，matcher，\code{fix} & 完了 & 必須\code{else}付き\code{matchFirst}を含む全M4構文で，公開推論の健全性・完全性・受理同値・決定可能性・主要性・主要型の一意性を検証済み．Paper 1の大規模な正確計算回帰も現仕様へ固定した．\\
M5 & 評価と型安全性 & 進行中 & core評価，順序付きmatching探索，runtime typing，\code{matchAll}／必須\code{else}付き\code{matchFirst}を含む共通fuel安全性まで進んだ．ここで型安全性とは，型の付いた実行が型の不整合で停止しない性質である．一般のM4静的型付けからruntime typingへの橋，多相\code{let}，任意のuser matcher branchとMNodeを覆う一般証明は残っている．\\
\bottomrule
\end{longtable}
...
M3固有の宣言・elaborationとM2--M3全域の公開静的定理は完了している．pattern/matcherを使う
論文listingはM4の回帰として分けて検証し，そちらも現行の必須\code{else}構文へ同期済みである．

\section{M4：patternとmatcher}

source構文は\code{fixE}，\code{matcher}，\code{matchAll}，必須の\code{else}を持つ
\code{matchFirst}，
...
成功から\code{M4.Typing}を得る健全性を証明済みである．\code{matchFirst}では通常armを
1本以上要求し，どのarmも結果を返さなければ，通常armが導入する変数を含まない元contextで
型付けした必須\code{else}式を使う．
...
さらに，異なる正しいelaboration導出を比較するcoherenceと，関係的導出を公開推論器の計算へ
対応付けるreplayを，全M4構文の帰納法へ接続した．その結果，\code{M4.Typing.infer\_isSome}による
完全性，\code{M4.typable\_iff\_infer\_isSome}による受理同値，\code{M4.typableDecidable}による
決定可能性，\code{M4.infer\_success\_principalResult}による主要性，および主要型の有限な変数名変更を
除いた一意性まで証明済みである．これらの公開入口は\path{TypePM/Source/FullM4Completion.lean}に
集約している．pattern functionの全具体化に対するinterface主要性は，このM4本体の完了とは別の
追加課題であり，ここでは完了を主張しない．
...
大きいmatcherの検証例（fixture）では，\code{M4.infer}の実行結果を\code{\#eval}で確認するだけでなく，
Lean kernelが検査する等式として期待結果を固定する必要がある．Lean kernelとは，作られた証明が
Leanの規則に従うかを最後に確認する，小さな検査器である．測度に基づく再帰で定義したpattern検査や
単一化は，単純な定義簡約だけでは大きな計算を止めることがある．

現行実装では，fuel付きelaborationをsolverまで引数化し，その成功を公開版へ移す定理を整備した．
これによりPaper 1のlist matcherとclosed multisetについて，正確な公開\code{M4.infer}結果と
\code{M4.Typing}をkernel回帰として固定し，依存先を与えないopen multisetの拒否も証明した．
したがってこれはM4の残作業ではない．前節のbranch-stable拒否は，計算量ではなく，公開推論が
hard制約にない型の外形を推測しないという別の仕様境界である．
...
\section{M4の静的定理の次はM5}

M2のill-formed開始supplyは仕様外であり，完了のために追加証明する項目ではない．
M4は，Paper 1の大規模な正確計算回帰と，全構文の完全性・受理同値・決定可能性・主要性まで
完了している．次の主作業はM5である．sourceの多相\code{let}，一般のM4 matcher導出，
user-defined matcher clause，pattern functionのMNodeをruntime typingへ構造的に接続し，
個別の値や一度の正確な実行結果に依存しない型保存と，型の不整合による停止が決して起きない性質
（no-stuck）を得る必要がある．core断片と具体的なPaper 1回帰では既に多くの安全性を証明しているが，
任意のM4 source導出から必要なruntime証明を自動構成する一般定理は未完である．
...
型等式を解く前に判定できる構文条件をまとめた検査である．ここでcatch-all最後尾という条件は
matcher literalのclause列に対するものであり，\code{matchFirst}の通常arm列に対する条件ではない．
$E$は各clauseが示した分解能力の証拠で，
...
表面構文は次のように，1本以上の通常armと，そのarmのbindingを受け取らず元contextで型付けする
必須\code{else}式を持つ．意味上の形は
\code{matchFirst target matcher nonemptyArms elseExpr}である．core/source ASTのconstructorは
\code{.matchFirst target matcher arms elseExpr}という四引数であり，静的規則が\code{arms}の非空性を課す．

\begin{lstlisting}
matchFirst target as matcher with
  | pattern1 -> body1
  | pattern2 -> body2
else elseExpr
\end{lstlisting}

制約生成規則の要点は次のとおりである．$F$は\code{else}式の生成結果，$A$は通常arm列の
生成結果である．

\[
\frac{
  \begin{gathered}
  \mathit{arms}=a_0::\mathit{rest},\\
  \Sigma;\Gamma;s\vdash e_t\Longrightarrow T;s_1,\qquad
  \Sigma;\Gamma;s_1\vdash e_m\Longrightarrow M;s_2,\\
  \Sigma;\Gamma;s_2\vdash e_{else}\Longrightarrow F;s_3,\\
  \Sigma;\Gamma;t_T;t_M;t_F;s_3\vdash_{tail}\mathit{arms}
    \Longrightarrow A;s'
  \end{gathered}
}{
  \begin{gathered}
  \Sigma;\Gamma;s\vdash
    \mathsf{matchFirst}(e_t,e_m,\mathit{arms},e_{else})
      \Longrightarrow G;s',\\
  G=\mathsf{fromMatchFirst}(T,M,\mathsf{fromFallback}(F,A))
  \end{gathered}
}\;\textsc{MatchFirst}
\]

通常arm列は空であってはならないが，最後をwildcardにする必要はない．各armではpattern targetを
$t_T$へhard等式でそろえ，matcher $t_M$をpattern capabilityのslotとしてpending checkingする．
\code{else}は通常armのpattern bindingを追加しない元のcontext $\Gamma$で調べ，そのtarget $t_F$を
全体結果型とする．全通常arm bodyのtargetは$t_F$へhard等式でそろえる．したがって\code{else}から
その時点のarm bindingを参照することはできない．ただし\code{else}も通常の式なので，その内部に別の
\code{matchAll}や\code{matchFirst}を書き，そこで新しく導入したbindingを対応するbodyから使うことはできる．
\code{matchAll}と違い，全体結果型へ\code{List}を付けない．

実行時にはtarget，matcherをこの順で一度ずつ先に評価し，通常armをsource順に試す．あるarmの探索が
非空なら，その先頭のbinding列を元の実行環境の前へ追加してbodyを評価する．空なら次のarmへ進み，
すべての通常armが空結果なら，\code{else}を元の実行環境で評価する．user matcherはwildcardに対しても
分解結果を0件返せるため，wildcard armは\code{else}の代用にならない．静的に受理されたsource式の
実行では，runtime helperが受け取る空arm列は，sourceで最初からarmを省略する構文ではなく，
全通常armを試し終えた後の内部終端だけを表す．targetまたは
matcherの評価が\code{stuck}または\code{timeout}なら，\code{else}を先に実行せず，その結果を伝える．
...
source構文の型規則とは分けている．M4については，この生成規則の健全性に加え，全構文のcoherenceと
replayを通して，公開推論の完全性・受理同値・決定可能性・主要性・主要型の一意性まで証明済みである．
一方，この静的な完了から，任意のM4式に対する一般のruntime型安全性が自動的に従うわけではない．
source導出からruntime typingとmatching証明書を構成するM5の一般橋は未完である．
\end{warningbox}
...
本書は2026年8月21日に，\code{type-pm-mech2}の現行実装，\code{README.md}，
および公開監査項目を基準に更新した．
