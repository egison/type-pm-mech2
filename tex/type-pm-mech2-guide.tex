...
M5 & 評価と型安全性 & 一部 & 順序付き探索，型付き環境，通常・再帰closure，直接のlambda・fix適用まで実装済み．ここで型安全性とは，型の付いた実行が型の不整合で停止しない性質である．高階関数，多相let，map，user-defined matcherを含む一般の場合が残る．\\
\bottomrule
\end{longtable}
...
型安全性は整数，真偽値，List，tuple，型付き環境，通常・再帰closure，直接のlambda・fix適用，
および明示的なcallback仮定の下のbuilt-in matcher断片までである．
callbackとは，処理本体を引数として受け取り，必要な時点で呼び出す関数である．built-in matcherとは，
利用者がsourceで定義するのではなく，実行器に直接実装されたmatcherである．
sourceの多相型付けからclosure断片への橋，高階の関数位置，map，user-defined matcher clause，
...
その後，M4全域の完全性・主要性を別計画として調べる．M5ではsourceの多相型付けからclosure断片への橋，
高階関数，map，user-defined matcher clause，pattern functionを一般の実行時型付けと，型の不整合による停止が
