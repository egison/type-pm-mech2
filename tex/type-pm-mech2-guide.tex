...
\begin{enumerate}[leftmargin=2em]
  \item 式を調べると得られる\code{target}，\code{hard}，\code{pending}とは何か．
  \item 多相\code{let}で右辺を先に閉じると，なぜ完全性の証明が難しくなるのか．
  \item 現行実装はその問題をどう解き，どこまでを証明済みとしているのか．
\end{enumerate}

後半では，Paper 1用fixtureとして実装されている再帰的multiset matcher，
とくに\code{multisetWithListArgument = lambda list. multiset}という補助式の意味と，
この式が現在のno-guess方針の下で単独では推論に失敗する理由も説明する．
...
記述が残っているため，現行mechanizationのAPI一覧としては使わない．
現状確認には本書，リポジトリ直下の\code{README.md}，\code{DESIGN.md}，
\code{CHECKPOINT.md}，およびLeanコードを用いる．
\end{warningbox}
...
健全性や完全性のような性質をLeanの定理として検査することをいう．

通常の関数型言語では，型推論は式に\code{Int}や
$\alpha\fn\beta$のような型を付ける．Type-PMではさらにmatcherを扱う．
...
M2 & scheme，多相\code{let} & 完了 & 一般の入れ子\code{letE}を含むM2--M3 sourceで完全性・受理同値・主要性を検証済み．\\
M3 & data，primitive，signature & 一部 & Bool/List，constructor，primitive，\code{ifE}と有限signatureを実装済み．M4と結ぶlisting全体の回帰が残る．\\
M4 & pattern，matcher，\code{fix} & 一部 & 最終構文と再帰dispatcherは実装済み．大きいfixtureのkernel推論回帰とM4全域の完全性・主要性が残る．\\
M5 & 評価と型安全性 & 一部 & 順序付き探索と多くの実行例を実装済み．closureやuser-defined matcherを含む一般の型安全性が残る．\\
\bottomrule
\end{longtable}

\begin{pointbox}{「M2が完了」の正確な意味}
M2の完全性・主要性は，開始supplyがcontextに対してwell-formedである範囲で完了している．
supplyは新しい型変数名を割り当てるカウンタである．公開推論は常に
\code{context.initialSupply}から始めるため，この条件を満たす．
well-formedでない任意の開始supplyは「未解決の宿題」ではなく，仕様の対象外である．
\end{pointbox}
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item \textbf{独立した型付け}：実行可能推論器を定義に含めない関係\code{Typing}で，
        source programが型を持つことを述べる．
\end{enumerate}

健全性は「推論が成功したなら独立した型付けが存在する」と述べる．
完全性は逆に「独立した型付けが存在するなら推論は成功する」と述べる．
主要性は，推論が返す型から代入によって他の型付け結果を得られることを述べる．
...
生成時点で要求の最外形が十分に分からない場合，特殊変換を推測すると，処理順により別の
結果を選ぶ危険がある．現行設計は，まずhardを最も一般的に解き，その結果でcheckingの種類を
分類する．この方針を本書では\emph{no-guess}，すなわち構造を先回りして推測しない方針と呼ぶ．
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item 右辺のhardを解き，pendingを分類・解決して，主要closureを得る．
  \item closureの代入を外側contextへ適用する．
  \item closureの結果型を，更新後contextに自由でない変数について一般化し，schemeを作る．
  \item そのschemeをcontext先頭へ追加してbodyを生成する．
  \item 右辺の内部制約は親blockへ出さず，外側contextへの有限な影響だけを
        interface equationsとして返す．
\end{enumerate}
...
主要closureは二段階の解をまとめる．

\begin{enumerate}[leftmargin=2em]
  \item hard制約を飽和させる最も一般的な代入．
  \item 残ったpendingから作る残余等式を解く最も一般的な代入．
\end{enumerate}

二つの代入を合成したものがclosure substitutionであり，それを\code{target}へ適用した型が
closure targetである．\emph{吸収的}であるとは，後から同じ制約を解く代入を合成しても，
...
一般化は，結果型に現れる変数のうち，更新後contextに自由に現れないものだけを量化する．
scheme内の量化変数は名前ではなく位置で表す．これをbound-index schemeという．
...
別名等式，それらを外から観測できなくするfreshness，frameのsupportをまとめる．
...
有限graphとして取り出す．これをvisible closure graphという．
...
\code{add}，\code{append}，\code{member}，\code{deleteFirst}，\code{map}のcanonical scheme，
...
value pattern，pattern constructor，pattern function applicationまで持つ．
...
matcher literalのstatic check，\code{fix}直下matcherの型形状，clause内部のslot要求，
および\code{let}での任意closure選択を同時に扱う必要があるため，独立した大きな課題である．

\section{M5：評価と型安全性}

M5には一般のruntime value，closure，matcher closure，順序付き深さ優先探索，
\code{matchAll}と\code{matchFirst}の関係的評価とfuel付き評価器がある．
fuelは再帰計算を進められる回数の上限である．有限な成功導出について，実行可能評価の健全性，
完全性，fuel単調性を証明している．

型安全性は閉じた整数と再帰的tuple，および明示的なcallback仮定の下のbuilt-in matcher断片までである．
closureの定義環境，primitiveの一般値上の型保存，user-defined matcher clause，data constructor，
一般pattern functionを同じ契約へ接続する作業が残る．これは標準的な保存・進行の形を持つが，
...
現行source ASTには，top-levelのuser-defined値を名前で参照するmodule環境がない．
...
\code{def multiset m := ...}に対応する．\code{fixE}を適用すると，実行時環境は
...
空のsource contextから使える閉じた式にする．静的確認だけなら，\code{multisetDefinition}を
既知のlist schemeを持つcontextで調べてもよい．end-to-endの実行回帰では隠れたprimitive値を使わず，
空context・空runtime environmentから始めるため，この明示的な適用を置いている．
...
大きいmatcher fixtureでは\code{M4.infer}の実行結果を\code{\#eval}で確認できても，
Lean kernelの定義簡約だけで同じ等式を\code{rfl}により証明することが難しい．
...
次の静的な主作業は，大きいPaper 1 fixtureをkernelで計算可能な形にし，正例の正確な
\code{M4.infer}結果と\code{M4.Typing}，負例の非型付けを固定することである．

その後，M4全域の完全性・主要性を別計画として調べる．M5ではclosure，data，primitive，
user-defined matcher clause，pattern functionを一般の実行時型付けとno-stuckへ接続する．
...
本書は2026年8月20日に，\code{type-pm-mech2}の\code{main}，
本文作成開始時commit \code{7fe112f}を基準に作成した．
文書追加以外の実装変更が入った場合は，\code{README.md}，\code{DESIGN.md}，
\code{CHECKPOINT.md}，\code{TypePM/AxiomAudit.lean}と照合して更新する．
