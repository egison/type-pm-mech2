...
M4の実dispatchと二index DFSの間には，一段だけのuser-matcher bridgeも追加した．実際の
`dispatchMatcherClauses`がtimeoutまたは型付きbranchを返す証明，実runtime環境でのbuilt-in miss，
各branchの直後の状態を一段小さい探索fuelで作るproducerから，`evaluationAtomReducer`の局所certificateを
構成する．探索全体の結果や式評価全体の安全性，従来の構造的な環境／値型付けを直接の前提にしない．
Paper 1の実multiset variable節では，再帰list closureをatom環境とtargetに置き，実dispatchの一段後まで
このbridgeで接続した．ただし現在importするM4 branch-work関係は，次の`.something` atomを正の探索indexで
型付けするときに，そのclosureへ従来の`ValueTyping`を要求する．この不可能性も定理化したため，現bridgeから
成功する三状態探索まで型保存を延ばしたとは主張しない．次はbranch-work自体をcaller指定の添字付き関係へ
一般化する必要がある．
...
2. `TotalCoreTyping.commonFuelSafety`を壊して前提を一つだけ交換するのではなく，環境・成功値・探索回答の
   ...
   関係と二つのindexを持つDFS定理まで追加済みである．次は，再帰closureを許す入力関係の下でuser matcherの
   workと一段のevaluator／dispatcher保存を構成し，この探索定理を式評価全体へ接続する．実dispatchを二indexの
   一段certificateへ変換するbridgeは追加済みだが，正の探索indexを持つbranch-workには従来の値型付けが残る．
   これをcaller指定の添字付き関係へ一般化し，P1-L01で既に証明した三種類のatom関係をfixture固有の局所証明では
   なくM4 clause導出から生成する経路へ置き換える．
...
| M4 user dispatchから二index DFSへの一段bridge | [M4TwoIndexUserMatcherReducerBridge.lean](TypePM/Source/M4TwoIndexUserMatcherReducerBridge.lean)，[M4TwoIndexUserMatcherReducerBridgeRegression.lean](TypePM/Source/M4TwoIndexUserMatcherReducerBridgeRegression.lean) |
