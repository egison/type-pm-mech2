...
| 5.6 | 静的型付けを状態を含まないruntime typingへ移す | **in progress** | 固定signatureのsource橋，共有M2--M3構文，通常のM4 `fixE`，解決済みM4 matcher clause，built-in pattern断片の必須`fallback`付き`matchFirst`からruntime証明書への橋は完了．source多相`let`は，実際のvalue elaborationと同じprincipal closureに対するcertificateがあれば任意valueを扱う．三つの代表回帰では，rootの通常等式条件も公開`infer`成功から自動構成する．この条件を対象fragment全体へ広げる構造的定理，matcher-rootの`fixE`，user matcher／constructorを含む再帰的M4式が残る |
...
4. 多相`let`のsame-closure certificateを，現在のidentityとclosed tuple以外の値についてもsource導出から
   ...
   保つ．三回帰で完成したroot残存検査の自動証明を，対象fragment全体へ構造的に広げる．
